...
cleanup() {
    ...
    if docker ps -q --filter "name=micromicro-test" | grep -q .; then
        docker stop $(docker ps -q --filter "name=micromicro-test")
    fi
    
    # Remover contenedores si existen
    if docker ps -aq --filter "name=micromicro-test" | grep -q .; then
        docker rm $(docker ps -aq --filter "name=micromicro-test")
    fi
...
start_test_services() {
    ...
    if ! docker network ls | grep -q "micromicro-test-net"; then
        docker network create micromicro-test-net
    fi
    ...
    timeout=30
    counter=0
    while ! docker exec micromicro-test-db pg_isready -U postgres &> /dev/null; do
    ...
    docker exec kafka-test kafka-topics --create --if-not-exists \
        ...
        --partitions 1 \
        --topic micromicro.created
    
    docker exec kafka-test kafka-topics --create --if-not-exists \
        --bootstrap-server localhost:9092 \
        --replication-factor 1 \
        --partitions 1 \
        --topic micromicro.updated
...
docker-compose -f docker-compose.test.yml up -d micromicro-test-db kafka-test

# Wait for database to be ready
echo "⏳ Waiting for database to be ready..."
until docker exec micromicro-test-db pg_isready -U postgres > /dev/null 2>&1; do
...
export project_name_screaming_DATABASE_URL="postgresql://postgres:postgres@localhost:5434/micromicro_test"
