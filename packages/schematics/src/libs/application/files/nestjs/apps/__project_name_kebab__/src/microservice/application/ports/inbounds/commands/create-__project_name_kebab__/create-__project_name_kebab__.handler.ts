 ...
 * -----------------------------------------------------------------------------
 * Handler que responde al comando `Create__project_name_camel__Command`.
 *
 * Se encarga de:
 ...
 * - Aplicar el evento `__project_name_camel__CreatedEvent`
 *
 * Esta clase forma parte de la **capa de aplicación**, implementando el patrón **CQRS**.
 *
 * Flujo de ejecución:
 * 1. El __project_name_camel__ o servicio envía `Create__project_name_camel__Command` al `CommandBus`
 * 2. NestJS ejecuta este handler
 * 3. Se crea un nuevo `__project_name_pascal__Entity` con sus reglas de dominio
 * 4. El handler invoca `create__project_name_camel__Port.create(...)`
 * 5. Se aplica el evento de creación (`entity.commit()`)
 * 6. Se retorna la entidad (puede ser transformada antes de exponerse)
 *
 * Dependencias:
 * - `Create__project_name_camel__Port`: capa de persistencia orientada a escritura
 */

import { CommandHandler, EventPublisher, ICommandHandler } from '@nestjs/cqrs'
import { Inject } from '@nestjs/common'

import { Create__project_name_camel__Command } from './create-__project_name_kebab__.command'
...
@CommandHandler(Create__project_name_camel__Command)
export class Create__project_name_camel__Handler
  implements ICommandHandler<Create__project_name_camel__Command>
{
  constructor(
    @Inject(Create__project_name_camel__Port)
    private readonly create__project_name_camel__Port: Create__project_name_camel__Port,
   ...
   */
  async execute(
    command: Create__project_name_camel__Command
    ...
    await this.create__project_name_camel__Port.create(merged)
    merged.commit()
