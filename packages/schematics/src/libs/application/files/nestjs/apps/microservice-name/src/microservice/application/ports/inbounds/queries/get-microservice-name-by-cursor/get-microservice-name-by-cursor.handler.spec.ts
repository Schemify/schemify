...
import { GetMicroserviceNameByCursorHandler } from './getmicroservice-name-by-cursor.handler'
import { GetMicroserviceNameByCursorQuery } from './getmicroservice-name-by-cursor.query'
...
describe('GetMicroserviceNameByCursorHandler', () => {
  ...
  describe('execute', () => {
    ...
    it('should return empty results when no data exists', async () => {
      ...
      const query = new GetMicroserviceNameByCursorQuery({
        afterId: 'lastmicroservice-name-id',
        limit: 10
      })
      ...
      expect(
        getMicroserviceNameWithCursorPort.getWithCursor
      ).toHaveBeenCalledWith('lastmicroservice-name-id', 10)
    })
    ...
    it('should handle first page request (no afterId)', async () => {
      ...
      const mockResult = {
        items: mockMicroserviceNames,
        nextCursor: 'segundomicroservice-name-id',
        hasMore: true
      }
      ...
      // Assert
      expect(result.items).toHaveLength(2)
      expect(result.nextCursor).toBe('segundomicroservice-name-id')
    ...
    it('should handle last page (no more data)', async () => {
      ...
      const query = new GetMicroserviceNameByCursorQuery({
        afterId: 'penultimomicroservice-name-id',
        limit: 5
      })
      ...
      expect(
        getMicroserviceNameWithCursorPort.getWithCursor
      ).toHaveBeenCalledWith('penultimomicroservice-name-id', 5)
    })
