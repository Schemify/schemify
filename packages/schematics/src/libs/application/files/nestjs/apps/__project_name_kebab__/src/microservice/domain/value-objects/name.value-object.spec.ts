...
describe('NameValueObject', () => {
  describe('create', () => {
    it('should create name with valid value', () => {
      // Arrange & Act
      const name = NameValueObject.create('__project_name_camel__ Test')

      // Assert
      expect(name.value).toBe('__project_name_camel__ Test')
    })

    it('should trim whitespace from name', () => {
      // Arrange & Act
      const name = NameValueObject.create(
        '  __project_name_camel__ con espacios  '
      )

      // Assert
      expect(name.value).toBe('__project_name_camel__ con espacios')
    })
  ...
  describe('equals', () => {
    it('should return true for same values', () => {
      // Arrange
      const name1 = NameValueObject.create('__project_name_camel__ Test')
    ...
    it('should return false for different values', () => {
      // Arrange
      const name1 = NameValueObject.create('__project_name_camel__ Test')
      const name2 = NameValueObject.create('__project_name_camel__ Diferente')
  ...
  describe('edge cases', () => {
    it('should handle special characters', () => {
      // Arrange & Act
      const name = NameValueObject.create('__project_name_camel__ @#$%^&*()')

      // Assert
      expect(name.value).toBe('__project_name_camel__ @#$%^&*()')
    })

    it('should handle numbers in name', () => {
      // Arrange & Act
      const name = NameValueObject.create('__project_name_camel__ 123')

      // Assert
      expect(name.value).toBe('__project_name_camel__ 123')
    })

    it('should handle unicode characters', () => {
      // Arrange & Act
      const name = NameValueObject.create(
        '__project_name_camel__ Español ñáéíóú'
      )

      // Assert
      expect(name.value).toBe('__project_name_camel__ Español ñáéíóú')
    })
  })
})
