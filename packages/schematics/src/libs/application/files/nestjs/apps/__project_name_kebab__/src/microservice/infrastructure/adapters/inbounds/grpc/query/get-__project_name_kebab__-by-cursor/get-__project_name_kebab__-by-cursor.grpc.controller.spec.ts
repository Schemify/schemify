...
import { Get__project_name_camel__ByCursorGrpcController } from './get__project_name_kebab__-by-cursor.grpc.controller'
...
describe('Get__project_name_camel__ByCursorGrpcController', () => {
  let controller: Get__project_name_camel__ByCursorGrpcController
  let queryBus: jest.Mocked<QueryBus>
  let mapper: jest.Mocked<__project_name_pascal__Mapper>

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [Get__project_name_camel__ByCursorGrpcController],
      providers: [
        {
          provide: QueryBus,
          useValue: {
    ...
    controller = module.get<Get__project_name_camel__ByCursorGrpcController>(
  ...
  describe('get__project_name_camel__ByCursor', () => {
    it('should return paginated __project_name_camel__ with next cursor', async () => {
      ...
      const mockCursorResult: CursorResult<__project_name_camel__Entity> = {
        items: mockEntities,
      ...
      const expectedResponse: __project_name_camel__.CursorPaginated__project_name_camel__ =
        {
      ...
      // Act
      const result = await controller.get__project_name_camel__ByCursor(request)

      // Assert
      // eslint-disable-next-line @typescript-eslint/unbound-method
      expect(queryBus.execute).toHaveBeenCalledWith(
        expect.any(Get__project_name_camel__ByCursorQuery)
      )
      // eslint-disable-next-line @typescript-eslint/unbound-method
      expect(mapper.entityToProto).toHaveBeenCalledTimes(2)

      const executedQuery = queryBus.execute.mock
        .calls[0][0] as Get__project_name_camel__ByCursorQuery
    ...
    it('should return paginated __project_name_camel__ without next cursor', async () => {
      ...
      const mockEntities = [
        __project_name_camel__Entity.create({
          name: '__project_name_camel__ Final',
    ...
    it('should handle query bus errors', async () => {
      ...
      // Act & Assert
      await expect(
        controller.get__project_name_camel__ByCursor(request)
  ...
  describe('constructor and dependencies', () => {
    ...
    it('should have correct method signature', () => {
      ...
      const method = (
        ...
        return controller.get__project_name_camel__ByCursor(request)
      }

      // Assert
      expect(typeof method).toBe('function')
      expect(controller.get__project_name_camel__ByCursor.name).toBe(
