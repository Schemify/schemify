...
import { __project_name_camel__Kafka } from '@nestjs/microservices'
import { of, throwError } from 'rxjs'

describe('KafkaProducerService', () => {
  let service: KafkaProducerService
  let mock__project_name_camel__Kafka: jest.Mocked<__project_name_camel__Kafka>

  beforeEach(async () => {
    const mock__project_name_camel__ = {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        KafkaProducerService,
        {
          ...
          useValue: mock__project_name_camel__
        }
      ]
    }).compile()

    service = module.get<KafkaProducerService>(KafkaProducerService)
    mock__project_name_camel__Kafka = module.get('KAFKA_PRODUCER')
  })
  ...
  describe('onModuleInit', () => {
    it('should connect to kafka on module init', async () => {
      ...
      // Assert
      expect(mock__project_name_camel__Kafka.connect).toHaveBeenCalledTimes(1)
    ...
    it('should not connect twice if already connected', async () => {
      ...
      // Assert
      expect(mock__project_name_camel__Kafka.connect).not.toHaveBeenCalled()
    })

    it('should handle connection errors', async () => {
      // Arrange
      const connectionError = new Error('Connection failed')
      mock__project_name_camel__Kafka.connect.mockRejectedValue(connectionError)
  ...
  describe('emit', () => {
    it('should emit message successfully', async () => {
      ...
      mock__project_name_camel__Kafka.emit.mockReturnValue(of(expectedResult))

      // Act
      const result = await service.emit(topic, payload)

      // Assert
      expect(mock__project_name_camel__Kafka.emit).toHaveBeenCalledWith(
        topic,
        payload
      )
    ...
    it('should handle emit errors', async () => {
      ...
      mock__project_name_camel__Kafka.emit.mockReturnValue(
    ...
    it('should handle connection errors during emit', async () => {
      ...
      expect(mock__project_name_camel__Kafka.emit).not.toHaveBeenCalled()
    })
  })
  ...
  describe('onApplicationShutdown', () => {
    it('should close kafka connection on shutdown', async () => {
      ...
      // Assert
      expect(mock__project_name_camel__Kafka.close).toHaveBeenCalledTimes(1)
    })
    ...
    it('should not close if not connected', async () => {
      ...
      // Assert
      expect(mock__project_name_camel__Kafka.close).not.toHaveBeenCalled()
    })
    ...
    it('should handle close errors gracefully', async () => {
      ...
      mock__project_name_camel__Kafka.close.mockRejectedValue(closeError)
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle multiple emits with single connection', async () => {
      ...
      // Assert
      expect(mock__project_name_camel__Kafka.connect).toHaveBeenCalledTimes(1)
      expect(mock__project_name_camel__Kafka.emit).toHaveBeenCalledTimes(2)
      expect(mock__project_name_camel__Kafka.emit).toHaveBeenNthCalledWith(
        1,
        topic1,
        payload1
      )
