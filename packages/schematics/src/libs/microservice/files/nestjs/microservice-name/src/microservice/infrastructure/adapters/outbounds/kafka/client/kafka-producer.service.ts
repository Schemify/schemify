  ...
  implements OnModuleInit, OnApplicationShutdown
{
  ...
  constructor(
    ...
    private readonly microserviceName: ClientKafka
  ) {}

  /** Conecta el producer (idempotente) */
  async onModuleInit(): Promise<void> {
    if (!this.ready) {
      await this.microserviceName.connect()
      this.ready = true
    }
  }
  ...
  async emit(topic: string, payload: any): Promise<unknown> {
    ...
    return lastValueFrom(this.microserviceName.emit(topic, payload))
  }

  /** Cierra la conexión al terminar la aplicación */
  async onApplicationShutdown(): Promise<void> {
    if (this.ready) await this.microserviceName.close()
  }
}
