...
import { Update__project_name_pascal__GrpcController } from './update-__project_name_kebab__.grpc.controller'
import { Update__project_name_pascal__Command } from '@__project_name_kebab__/microservice/application/ports/inbounds/commands'
...
import { NameValueObject } from '@__project_name_kebab__/microservice/domain/value-objects/name.value-object'
import { DescriptionValueObject } from '@__project_name_kebab__/microservice/domain/value-objects/description.value-object'

describe('Update__project_name_pascal__GrpcController', () => {
  let controller: Update__project_name_pascal__GrpcController
  let commandBus: jest.Mocked<CommandBus>
  let mapper: jest.Mocked<__project_name_pascal__Mapper>

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      controllers: [Update__project_name_pascal__GrpcController],
      providers: [
        {
          provide: CommandBus,
          useValue: {
    ...
    controller = module.get<Update__project_name_pascal__GrpcController>(
  ...
  describe('update__project_name_pascal__', () => {
    it('should update __project_name_kebab__ successfully with all fields', async () => {
      // Arrange
      const request: __project_name_kebab__.Update__project_name_pascal__Dto = {
        id: 'test-id-123',
        __project_name_kebab__: {
          name: 'Cliente Actualizado',
          description: 'Nueva descripción del cliente'
        }
      }

      const mockProps = {
        name: NameValueObject.create('Cliente Actualizado'),
        description: DescriptionValueObject.create(
          'Nueva descripción del cliente'
        )
      }
      ...
      // Act
      const result = await controller.update__project_name_pascal__(request)

      // Assert
      // eslint-disable-next-line @typescript-eslint/unbound-method
      expect(mapper.protoToProps).toHaveBeenCalledWith(request.__project_name_kebab__)
      // eslint-disable-next-line @typescript-eslint/unbound-method
    ...
    it('should update __project_name_kebab__ with only name field', async () => {
      // Arrange
      const request: __project_name_kebab__.Update__project_name_pascal__Dto = {
        id: 'test-id-456',
        __project_name_kebab__: {
    ...
    it('should update __project_name_kebab__ with only description field', async () => {
      // Arrange
      const request: __project_name_kebab__.Update__project_name_pascal__Dto = {
        id: 'test-id-789',
        __project_name_kebab__: {
    ...
    it('should throw error when __project_name_kebab__ object is missing', async () => {
      // Arrange
      const request = {
        id: 'test-id-123'
        // __project_name_kebab__ is missing
      } as __project_name_kebab__.Update__project_name_pascal__Dto

      // Act & Assert
      await expect(controller.update__project_name_pascal__(request)).rejects.toThrow(
        'Falta el objeto __project_name_kebab__ en el payload'
      )
    ...
    it('should handle command bus errors', async () => {
      ...
      const mockProps = {
        name: NameValueObject.create('Cliente Error'),
        description: DescriptionValueObject.create('Descripción')
      }

      const commandError = new Error('Client not found')
      mapper.protoToProps.mockReturnValue(mockProps)
    ...
    it('should handle mapper errors during entityToProto', async () => {
      ...
      const mockProps = {
        name: NameValueObject.create('Cliente Mapper Error'),
        description: DescriptionValueObject.create('Descripción')
      }
    ...
    it('should handle empty __project_name_kebab__ object with only name', async () => {
      // Arrange
      const request: __project_name_kebab__.Update__project_name_pascal__Dto = {
        id: 'test-id-123',
        __project_name_kebab__: {
  ...
  describe('constructor and dependencies', () => {
    it('should be properly instantiated with dependencies', () => {
      ...
      expect(controller).toBeInstanceOf(Update__project_name_pascal__GrpcController)
    ...
    it('should have correct method signature', () => {
      // Arrange
      const method = (request: __project_name_kebab__.Update__project_name_pascal__Dto) => {
        return controller.update__project_name_pascal__(request)
      }

      // Assert
      expect(typeof method).toBe('function')
      expect(controller.update__project_name_pascal__.name).toBe('update__project_name_pascal__')
    })
  })
})
