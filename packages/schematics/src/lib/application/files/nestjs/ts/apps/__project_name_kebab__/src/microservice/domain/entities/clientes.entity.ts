...
export class __project_name_pascal__Entity extends AggregateRoot {
   ...
   */
  static create(input: { name: string; description?: string }): __project_name_pascal__Entity {
    const now = new Date()
    ...
    return {
      ...
      description: (value: string) => {
        ...
        if (!this.props.description?.equals(newDesc)) {
          ...
          this.apply(new __project_name_pascal__DescriptionUpdatedEvent(this.id, newDesc))
        }
      }
    }
  }
