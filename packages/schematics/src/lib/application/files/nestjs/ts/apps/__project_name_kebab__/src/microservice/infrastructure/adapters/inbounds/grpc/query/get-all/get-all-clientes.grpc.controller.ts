...
export class GetAll__project_name_pascal__GrpcController {
   ...
   *  3. Se mapean a objetos ` __project_name_kebab__.__project_name_pascal__` (Protobuf)
   *  4. Se devuelve el objeto ` __project_name_kebab__.__project_name_pascal__` (con una lista de resultados)
   *
   * @returns Protobuf ` __project_name_kebab__.__project_name_pascal__` con la lista de ejemplos
   */
  @GrpcMethod('ClientesService', 'getAllClientes')
  async getAllClientes(): Promise<__project_name_pascal__List> {
    const entities = await this.queryBus.execute(new GetAll__project_name_pascal__Query())

    return {
      __project_name_camel__: entities.map((entity) => this.mapper.entityToProto(entity))
    }
  }
}
