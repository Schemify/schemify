...
import { Kafka__project_name_pascal__CreatedPublisher } from './__project_name_kebab__-created.adapter'
...
describe('Kafka__project_name_pascal__CreatedPublisher', () => {
  let publisher: Kafka__project_name_pascal__CreatedPublisher
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        Kafka__project_name_pascal__CreatedPublisher,
        {
    ...
    publisher = module.get<Kafka__project_name_pascal__CreatedPublisher>(
      Kafka__project_name_pascal__CreatedPublisher
    )
  ...
  describe('publish', () => {
    it('should publish __project_name_camel__ created event successfully', async () => {
      ...
      const event = new __project_name_pascal__CreatedEvent(__project_name_camel__)
      ...
      expect(emitSpy).toHaveBeenCalledWith('__project_name_kebab__.created', {
    ...
    it('should serialize event payload correctly', async () => {
      ...
      expect(messageValue.payload.__project_name_camel__.id).toBe(__project_name_camel__.id)
      expect(messageValue.payload.__project_name_camel__.props.name.value).toBe(
        'Test Cliente'
      )
      expect(messageValue.payload.__project_name_camel__.props.description?.value).toBe(
        'Test Description'
      )
    })
    ...
    it('should handle __project_name_camel__ without description', async () => {
      ...
      expect(emitSpy).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          key: __project_name_camel__.id,
          value: expect.stringContaining('Test Cliente')
        })
      )
    })
  })
