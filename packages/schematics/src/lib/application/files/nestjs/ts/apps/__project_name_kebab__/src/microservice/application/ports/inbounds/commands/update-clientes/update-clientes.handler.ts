  ...
  implements ICommandHandler<Update__project_name_pascal__Command>
{
   ...
   */
  async execute(command: Update__project_name_pascal__Command): Promise<__project_name_pascal__Entity> {
    const __project_name_kebab__ = await this.get__project_name_pascal__ByIdPort.getById(command.id)

    if (!__project_name_kebab__) {
      throw new NotFoundException(`__project_name_pascal__ with id ${command.id} not found`)
    }

    // ✅ Usamos el método de actualización centralizado en el agregado
    __project_name_kebab__.update({
      name: command.name,
      description: command.description
    })

    await this.update__project_name_pascal__Port.update(__project_name_kebab__)
    __project_name_kebab__.commit()

    return __project_name_kebab__
  }
}
