...
import { Get__project_name_pascal__ByCursorGrpcController } from './get-__project_name_kebab__-by-cursor.grpc.controller'
...
describe('Get__project_name_pascal__ByCursorGrpcController', () => {
  ...
  describe('get__project_name_pascal__ByCursor', () => {
    it('should return paginated __project_name_kebab__ with next cursor', async () => {
      // Arrange
      const request: __project_name_kebab__.CursorPaginationRequest = {
        afterId: 'client-123',
        limit: 10
      }
      ...
      const mockCursorResult: CursorResult<__project_name_pascal__Entity> = {
        items: mockEntities,
        nextCursor: 'client-456',
        hasMore: true
      }

      const expectedProto__project_name_pascal__ = mockEntities.map((entity) => ({
        id: entity.id,
        name: entity.props.name.value,
        description: entity.props.description?.value
      }))

      const expectedResponse: __project_name_kebab__.CursorPaginated__project_name_pascal__ = {
        __project_name_kebab__: expectedProto__project_name_pascal__,
        nextCursor: 'client-456',
        hasMore: true
      }
      ...
      // Act
      const result = await controller.get__project_name_pascal__ByCursor(request)
      ...
      expect(result.__project_name_kebab__).toHaveLength(2)
      expect(result.nextCursor).toBe('client-456')
      expect(result.hasMore).toBe(true)
    })

    it('should return paginated __project_name_kebab__ without next cursor', async () => {
      // Arrange
      const request: __project_name_kebab__.CursorPaginationRequest = {
        afterId: 'client-789',
        limit: 5
      ...
      const expectedResponse: __project_name_kebab__.CursorPaginated__project_name_pascal__ = {
        __project_name_kebab__: [expectedProtoCliente],
        nextCursor: '',
        hasMore: false
      }

      queryBus.execute.mockResolvedValue(mockCursorResult)
      mapper.entityToProto.mockReturnValue(expectedProtoCliente)
      ...
      expect(result).toEqual(expectedResponse)
      expect(result.__project_name_kebab__).toHaveLength(1)
    ...
    it('should handle empty results', async () => {
      ...
      expect(result).toEqual(expectedResponse)
      expect(result.__project_name_kebab__).toHaveLength(0)
      expect(result.nextCursor).toBe('')
      expect(result.hasMore).toBe(false)
    })
    ...
    it('should handle query bus errors', async () => {
      ...
      // Act & Assert
      await expect(controller.get__project_name_pascal__ByCursor(request)).rejects.toThrow(
    ...
    it('should handle large limit values', async () => {
      ...
      expect(result).toEqual(expectedResponse)
      expect(result.__project_name_kebab__).toHaveLength(100)
    })
  ...
  describe('constructor and dependencies', () => {
    it('should be properly instantiated with dependencies', () => {
      ...
      expect(controller).toBeInstanceOf(Get__project_name_pascal__ByCursorGrpcController)
    ...
    it('should have correct method signature', () => {
      // Arrange
      const method = (request: __project_name_kebab__.CursorPaginationRequest) => {
        return controller.get__project_name_pascal__ByCursor(request)
      }

      // Assert
      expect(typeof method).toBe('function')
      expect(controller.get__project_name_pascal__ByCursor.name).toBe('get__project_name_pascal__ByCursor')
    })
  })
})
