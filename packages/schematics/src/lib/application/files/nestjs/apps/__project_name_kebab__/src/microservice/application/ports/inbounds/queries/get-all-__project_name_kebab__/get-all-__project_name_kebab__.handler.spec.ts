...
import { GetAll__project_name_pascal__Handler } from './get-all-__project_name_camel__.handler'
...
describe('GetAll__project_name_pascal__Handler', () => {
  ...
  describe('execute', () => {
    it('should return all __project_name_camel__s when repository has data', async () => {
      ...
      // Assert
      expect(result).toBe(mock__project_name_pascal__s)
    ...
    it('should handle single __project_name_camel__ result', async () => {
      // Arrange
      const single__project_name_pascal__ = [
        __project_name_pascal__Entity.create({
      ...
      // Assert
      expect(result).toBe(single__project_name_pascal__)
      expect(result).toHaveLength(1)
      expect(result[0]).toBeInstanceOf(__project_name_pascal__Entity)
      expect(result[0].props.name.value).toBe('__project_name_pascal__ Único')
      expect(getAll__project_name_pascal__Port.getAll).toHaveBeenCalledTimes(1)
    })
    ...
    it('should handle large number of __project_name_camel__s', async () => {
      ...
      getAll__project_name_pascal__Port.getAll.mockResolvedValue(
        large__project_name_pascal__List
      )
      ...
      // Assert
      expect(result).toBe(large__project_name_pascal__List)
    ...
    it('should preserve __project_name_camel__ entity properties', async () => {
      ...
      const mock__project_name_pascal__s = [__project_name_camel__WithAllProps]
      getAll__project_name_pascal__Port.getAll.mockResolvedValue(
        mock__project_name_pascal__s
      )
      ...
      expect(result[0].props.name.value).toBe(
        '__project_name_pascal__ Completo'
      )
