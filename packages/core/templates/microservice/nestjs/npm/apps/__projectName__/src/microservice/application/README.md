# 🧠 Capa de Aplicación - Microservicio `__ProjectName__`
...
│   ├── 📁 create-__projectName__
│   │   ├── create-__projectName__.command.ts
│   │   └── create-__projectName__.handler.ts
│   ├── 📁 update-__projectName__
│   │   ├── update-__projectName__.command.ts
│   │   └── update-__projectName__.handler.ts
│   ├── 📁 delete-__projectName__
│   │   ├── delete-__projectName__.command.ts
│   │   └── delete-__projectName__.handler.ts
│   └── index.ts                → Exporta y registra todos los handlers

├── 📁 queries                   → Casos de uso que consultan el sistema (read)
│   ├── 📁 get-all-__projectName__s
│   ├── 📁 get-__projectName__-by-id
│   └── 📁 get-__projectName__s-by-cursor
│   └── index.ts                → Exporta y registra todos los handlers

├── 📁 events                   → EventHandlers que escuchan eventos del dominio
│   ├── 📁 __projectName__-created
│   ├── 📁 __projectName__-renamed
│   └── 📁 __projectName__-description-updated
...
* Transforman entidades del dominio (`__ProjectName__Entity`) a objetos de transporte (Protobuf, DTO)
